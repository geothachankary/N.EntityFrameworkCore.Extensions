...
public static class DbContextExtensions
{
    ...
    private static readonly ConcurrentDictionary<string, bool> mySqlBulkCopyAvailability = new();
    ...
    public static int BulkInsert<T>(this DbContext context, IEnumerable<T> entities, BulkInsertOptions<T> options)
    {
        ...
        using (var bulkOperation = new BulkOperation<T>(context, options, options.InputColumns, options.IgnoreColumns))
        {
            try
            {
                ...
                if (CanUseDirectBulkInsert(bulkOperation, options))
                {
                    var columnsToInsert = bulkOperation.GetColumnNames(keepIdentity);
                    rowsAffected = BulkInsert(entities, options, bulkOperation.TableMapping, bulkOperation.Connection, bulkOperation.Transaction,
                        bulkOperation.TableMapping.FullQualifedTableName, columnsToInsert).RowsAffected;
                }
                else
                {
    ...
    internal static bool CanUseDirectBulkInsert<T>(BulkOperation<T> bulkOperation, BulkInsertOptions<T> options)
    {
        if (options.InsertIfNotExists || options.InsertOnCondition != null)
            return false;

        if (bulkOperation.TableMapping.EntityTypes.Skip(1).Any())
            return false;

        return !options.AutoMapOutput || !bulkOperation.TableMapping.GetAutoGeneratedColumns().Any();
    }
    ...
    internal static bool CanUseMySqlBulkCopy(MySqlConnection mySqlConnection, MySqlTransaction transaction, BulkOptions options)
    {
        ...
        if (mySqlBulkCopyAvailability.TryGetValue(mySqlConnection.ConnectionString, out bool canUseBulkCopy))
            return canUseBulkCopy;
        ...
        canUseBulkCopy = Convert.ToInt32(command.ExecuteScalar()) == 1;
        mySqlBulkCopyAvailability[mySqlConnection.ConnectionString] = canUseBulkCopy;
        return canUseBulkCopy;
    }
